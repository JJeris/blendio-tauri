...
pub async fn insert_installed_blender_version(
    ...
    let parent_dir = match executable_file_path.parent() { // A (1.a.) let parent_dir =; C (3.b) match; B (2.a.) .parent()
    ...
    let dir_name = match parent_dir.file_name() { // A (1.a.) let dir_name =; C (3.b) match; B (2.a.) .file_name()
    ...
    let re = match Regex::new(r"blender-(?P<version>\d+\.\d+(?:\.\d+)?)-(?P<variant>[^\-+]+)") { // A (1.a.) let re =; C (3.b) match; B (2.a.) ...::new()
        Ok(val) => val, // C (3.c.) Ok()
        Err(err) => { // C (3.c) Err()
            show_ok_notification( // B (2.a.) show_ok_notification()
                app.clone(), // B (2.a.) app.clone();
    ...
    if let Some(caps) = re.captures(&dir_name) { // A (1.d.) if let Some(); B (2.a.) .captures()
    ...
    let entry = InstalledBlenderVersion { // A (1.a.) let entry =;
...
pub async fn insert_and_refresh_installed_blender_versions(
    ...
    let blender_repo_paths = match blender_repo_paths_repo.fetch(None, None, None).await { // A (1.a.) let blender_repo_paths =; C (3.b) match; B (2.a.) .fetch()
        Ok(val) => val, // C (3.c.) Ok()
        Err(err) => { // C (3.c) Err()
            show_ok_notification( // B (2.a.) show_ok_notification()
                app.clone(), // B (2.a.) app.clone();
            ...
            return Err(format!("Failed to fetch Blender repo paths: {:?}", err)); // B (2.b.) priekšlaicīgs return
        }
    };
    ...
    for repo_path in blender_repo_paths { // A (1.a.) let repo_path =;
        let directory_entries = match std::fs::read_dir(repo_path.repo_directory_path) { // A (1.a.) let directory_entries =; C (3.b) match; B (2.a.) ::read_dir()
            Ok(val) => val, // C (3.c.) Ok()
            Err(err) => { // C (3.c) Err()
                show_ok_notification( // B (2.a.) show_ok_notification()
                    app.clone(), // B (2.a.) app.clone();
        ...
        for entry in directory_entries { // A (1.a.) let entry =;
            let entry = match entry { // A (1.a.) let entry =; C (3.b) match
                Ok(val) => val, // C (3.c.) Ok()
                Err(err) => { // C (3.c) Err()
                    show_ok_notification( // B (2.a.) show_ok_notification()
                        app.clone(), // B (2.a.) app.clone();
            ...
            if !entry.path().is_dir() { // C (3.a.) entry.path().is_dir() != true; B (2.a.) .path(); B (2.a.) .is_dir(); 
            ...
            if !launcher_path.exists() { // C (3.a.) launcher_path.exists() != true; B (2.a.) .exists()
                continue; // B (2.b.) continue
            }
            ...
            if !existing_entries.is_empty() { // C (3.a.) existing_entries.is_empty() != true; B (2.a.) .exists()
                continue; // B (2.b.) continue
            }
    ...
    let current_entries = match installed_blender_version_repo.fetch(None, None, None).await { // A (1.a.) let current_entries =; C (3.b) match; B (2.a.) installed_blender_version_repo.fetch()
        Ok(val) => val, // C (3.c.) Ok()
        Err(err) => { // C (3.c) Err()
            show_ok_notification( // B (2.a.) show_ok_notification()
                app.clone(), // B (2.a.) app.clone();
    ...
    for entry in current_entries { // A (1.a.) let entry =;
        ...
        if !path.exists() { // C (3.a.) path.exists() != true; B (2.a.) .exists()
            match installed_blender_version_repo.delete(&entry.id).await { // C (3.b) match; B (2.a.) installed_blender_version_repo.delete()
                Ok(_) => {} // C (3.c.) Ok()
                Err(err) => { // C (3.c) Err()
                    show_ok_notification( // B (2.a.) show_ok_notification()
                        app.clone(), // B (2.a.) app.clone();
                    ...
                    return Err(format!("Failed to delete Blender version entry: {:?}", err)); // B (2.b.) priekšlaicīgs return
                }
            }
        }
    }
...
pub async fn update_installed_blender_version(
    ...
    let repository = InstalledBlenderVersionRepository::new(&state.pool); // A (1.a.) let repository =; B (2.a.) ...::new()
    let mut results = match repository.fetch(Some(&id), None, None).await { // A (1.a.) let mut results =; C (3.b) match
        Ok(val) => val, // C (3.c.) Ok()
        Err(err) => { // C (3.c) Err()
            show_ok_notification( // B (2.a.) show_ok_notification()
                app.clone(), // B (2.a.) app.clone();
    ...
    if results.is_empty() { // C (3.a) results.is_empty() == true; B (2.a.) .is_empty()
        show_ok_notification( // B (2.a.) show_ok_notification()
            app.clone(), // B (2.a.) app.clone();
    ...
    if is_default == true { // C (3.a) is_default == true
        entry.is_default = false; // A (1.a.) entry.is_default =;
        match repository.update(&entry).await { // C (3.b) match; B (2.a.) .update()
        ...
        let results = match repository.fetch(None, None, None).await { // A (1.a.) let results =; C (3.b) match; B (2.a.) .fetch(
            Ok(val) => val, // C (3.c.) Ok()
            Err(err) => { // C (3.c) Err()
                show_ok_notification( // B (2.a.) show_ok_notification()
                    app.clone(), // B (2.a.) app.clone();
        ...
        for mut entry in results { // A (1.a.) let mut entry =;
            let new_default = entry.id == id; // A (1.a.) let new_default =; C (3.a) entry.id == id
            if entry.is_default != new_default { // C (3.a) entry.is_default != new_default
...
pub async fn uninstall_and_delete_installed_blender_version_data(
    ...
    let confirmation = file_system_utility::show_ask_notification( // A (1.a.) let confirmation =; B (2.a.) ::show_ask_notification()
        app.clone(), // B (2.a.) app.clone();
    ...
    if installed_blender_version_list.is_empty() { // B (2.a.) .is_empty(); C (3.a) installed_blender_version_list.is_empty() == true
        show_ok_notification( // B (2.a.) show_ok_notification()
            app.clone(), // B (2.a.) app.clone();
            format!("Failed to fetch installed Blender version by ID"),
            tauri_plugin_dialog::MessageDialogKind::Error,
    ...
    .await
    {
        Ok(_) => {} // C (3.c.) Ok()
        Err(err) => { // C (3.c) Err()
            match repository.delete(&entry.id).await { // C (3.b) match; B (2.a.) .delete()
                Ok(_) => {} // C (3.c.) Ok()
                Err(err) => show_ok_notification(  // C (3.c) Err(); B (2.a.) show_ok_notification()
                    app.clone(), // B (2.a.) app.clone();
...
pub async fn launch_blender_version_with_launch_args(
    ...
    match installed_blender_version_repository.update(&instance).await { // C (3.b) match; B (2.a.) .update()
        Ok(_) => {} // C (3.c.) Ok()
        Err(err) => { // C (3.c) Err()
            show_ok_notification( // B (2.a.) show_ok_notification()
                app.clone(), // B (2.a.) app.clone();
    ...
    match launch_arguments_id { // C (3.b) match
        Some(arg_id) => { // C (3.c) Some()
            ...
            if launch_argument_entry_list.is_empty() { // B (2.a.) .is_empty(); C (3.a) launch_argument_entry_list.is_empty() == true
                show_ok_notification( // B (2.a.) show_ok_notification()
                    app.clone(), // B (2.a.) app.clone();
            ...
            match launch_argument_repository.update(&entry).await { // C (3.b) match; B (2.b.) .update()
                Ok(_) => {} // C (3.c.) Ok()
                Err(err) => { // C (3.c) Err()
                    show_ok_notification( // B (2.a.) show_ok_notification()
                        app.clone(), // B (2.a.) app.clone();
                    ...
                    return Err(format!("Failed to update launch argument: {:?}", err)); // B (2.b.) priekšlaicīgs return
                }
            }
    ...
    match python_script_id { // C (3.b) match
        Some(script_id) => { // C (3.c) Some()
            ...
            if python_script_entry_list.is_empty() { // B (2.a.) .is_empty(); C (3.a) python_script_entry_list.is_empty() == true
                show_ok_notification( // B (2.a.) show_ok_notification()
                    app.clone(), // B (2.a.) app.clone();
            ...
            match python_script_repository.update(&entry).await { // C (3.b) match; B (2.a.) .update()
                Ok(_) => {} // C (3.c.) Ok()
                Err(err) => { // C (3.c) Err()
                    show_ok_notification( // B (2.a.) show_ok_notification()
                        app.clone(), // B (2.a.) app.clone();
                    ...
                    return Err(format!("Failed to update python script: {:?}", err)); // B (2.b.) priekšlaicīgs return
                }
            }
...
pub async fn get_downloadable_blender_version_data(
    ...
    let response_json: Vec<DownloadableBlenderVersion> = match response.json().await { // A (1.a.) let response_json =; C (3.b.) match; B (2.a.) .json()
        Ok(val) => val, // C (3.c.) Ok()
        Err(err) => { // C (3.c) Err()
            show_ok_notification( // B (2.a.) show_ok_notification()
                app.clone(), // B (2.a.) app.clone();
...
pub async fn download_and_install_blender_version(
    ...
    let repository = InstalledBlenderVersionRepository::new(&state.pool); // A (1.a.) let repository =; B (2.a.) ...::new()
    let mut entry = InstalledBlenderVersion { // A (1.a.) let mut entry =;
        id: uuid::Uuid::new_v4().to_string(), // B (2.a.) uuid::Uuid::new_v4(); B (2.a.) .to_string();
    ...
    let installation_directory_path = match file_system_utility::extract_archive( // A (1.a.) let installation_directory_path =; C (3.b) match; B (2.a.) ::extract_archive()
    ...
    match file_system_utility::delete_file(archive_file_path).await { // C (3.b) match; B (2.a.) ::delete_file()
        Ok(_) => {} // C (3.c.) Ok()
        Err(err) => { // C (3.c) Err()
            show_ok_notification( // B (2.a.) show_ok_notification()
                app.clone(), // B (2.a.) app.clone();
    ...
    if existing_entries.is_empty() { // B (2.a.) .is_empty(); C (3.a) existing_entries.is_empty() == true
        ...
        match repository.update(&old_entry).await { // C (3.b) match; B (2.a.) .update()
            Ok(_) => Ok(()), // C (3.c.) Ok(); B (2.b.) priekšlaicīgs return
            Err(err) => { // C (3.c) Err()
                show_ok_notification( // B (2.a.) show_ok_notification()
                    app.clone(), // B (2.a.) app.clone();
...
pub async fn insert_blender_version_installation_location(
    ...
    let repo_directory_path = match repo_directory_path_option { // A (1.a.) let repo_directory_path =; C (3.b) match
        Some(val) => val, // C (3.c) Some()
        ...
        .await
    {
        ...
        Err(err) => { // C (3.c) Err()
            ...
            return Err(format!("Failed to fetch Blender repo path: {:?}", err)); // B (2.b.) priekšlaicīgs return
        }
    };
    if !results.is_empty() { // B (2.a.) .is_empty(); C (3.a) results.is_empty() != true
        return Ok(()); // B (2.b.) priekšlaicīgs return
    }
    let entry = BlenderRepoPath { // A (1.a.) let entry =;
        id: uuid::Uuid::new_v4().to_string(), // B (2.a.) uuid::Uuid::new_v4(); B (2.a.) .to_string();
    ...
    match repository.insert(&entry).await { // C (3.b.) match; B (2.a.) repository.insert();
        ...
        Err(err) => { // C (3.c) Err()
            ...
            return Err(format!("Failed to insert Blender repo path: {:?}", err)); // B (2.b.) priekšlaicīgs return
        }
    }
}
...
pub async fn update_blender_version_installation_location(
    ...
    let repository = BlenderRepoPathRepository::new(&state.pool); // A (1.a.) let repository =; B (2.a.) ...::new()
    let mut results = match repository.fetch(Some(&id), None, None).await { // A (1.a.) let mut results =; C (3.b) match; B (2.a.) .fetch()
        Ok(val) => val, // C (3.c.) Ok()
        Err(err) => { // C (3.c) Err()
            show_ok_notification( // B (2.a.) show_ok_notification()
                app.clone(), // B (2.a.) app.clone();
    ...
    if is_default == true { // C (3.a) is_default == true
        ...
        match repository.update(&entry).await { // C (3.b) match; B (2.a.) .update()
            ...
            Err(err) => { // C (3.c) Err()
                ...
                return Err(format!("Failed to update Blender repo path: {:?}", err)); // B (2.b.) priekšlaicīgs return
            }
        }
    } else { // C (3.b.) else
        let results = match repository.fetch(None, None, None).await { // A (1.a.) let results =; C (3.b) match; B (2.a.) .fetch()
            ...
            if entry.is_default != new_default { // C (3.a) entry.is_default != new_default
                ...
                match repository.update(&entry).await { // C (3.b) match; B (2.a.) .update()
                    ...
                    Err(err) => { // C (3.c) Err()
                        ...
                        return Err(format!("Failed to update Blender repo paths: {:?}", err)); // B (2.b.) priekšlaicīgs return
                    }
                }
            }
        }
...
pub async fn delete_blender_version_installation_location(
    ...
    if blender_repo_path_list.is_empty() { // B (2.a.) .is_empty(); C (3.a) blender_repo_path_list.is_empty() == true
        show_ok_notification( // B (2.a.) show_ok_notification()
            app.clone(), // B (2.a.) app.clone();
    ...
    for version in installed_blender_version_list { // A (1.a.) let mut version =;
        if version
    ...
    match blender_repo_path_repository.delete(&id).await { // C (3.b) match; B (2.a.) .delete()
        Ok(_) => Ok(()), // C (3.c.) Ok()
        Err(err) => { // C (3.c) Err()
            show_ok_notification( // B (2.a.) show_ok_notification()
                app.clone(), // B (2.a.) app.clone();
